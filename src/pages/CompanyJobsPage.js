...
const CompanyJobsPage = () => {
  ...
  useEffect(() => {
    const fetchCompanyJobs = async () => {
      try {
        const res = await axiosInstance.get(`/jobs/company/jobs/${companyId}`);
        setJobs(res.data);
                    ...
                    <div className="p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          {job.effectiveStatus === "DeadlineReached" && (
                            <span className="bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200 px-3 py-1 rounded-md text-xs font-medium">
                              Deadline Passed
                            </span>
                          )}
                          {job.effectiveStatus === "Closed" && (
                            <span className="bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 px-3 py-1 rounded-md text-xs font-medium">
                              Closed
                            </span>
                          )}
                        </div>
                        ...
                        {job.applicationDeadline && (
                          <div className="flex items-center">
                            <span className="text-xs text-gray-500">
                              Application deadline: {formatDateDMY(job.applicationDeadline)}
                            </span>
                          </div>
                        )}
