...
const CompanyProfilePage = () => {
  ...
  useEffect(() => {
    const fetchCompanyProfile = async () => {
      try {
  // Use shared axiosInstance (baseURL already includes /api) so this targets /api/companies/:id on backend
            ...
            <motion.div variants={itemVariants}>
              ...
              {userType === "user" && user && (
                <button
                  onClick={async () => {
                    try {
                      const convo = await startConversation("user", { companyId: company._id });
                      navigate("/chat");
                    } catch (e) {
                      console.error("Error starting conversation:", e);
                    }
                  }}
                  className="ml-3 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-300 inline-block"
                >
                  Start Chat
                </button>
              )}
